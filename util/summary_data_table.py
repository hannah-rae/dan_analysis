...
for soldir in sorted(glob('/Users/hannahrae/data/dan/dan_asufits_hiweh/*')):
    ...
    for mdir in glob(os.path.join(soldir, '*')):
        ...
        chi2_desc = glob(os.path.join(mdir, 'chi2_surface_*.png'))[0].split('/')[-1][:-4]
        bf_wehs.append(chi2_desc.split('_')[3])
        bf_bnacss.append(chi2_desc.split('_')[5][5:]) # includes the word BNACS oops
        # Get the uncertainty range for WEH
        unc_wehs.append(chi2_desc.split('_')[4])
        unc_bnacss.append(chi2_desc.split('_')[6])
...
np.savetxt('/Users/hannahrae/data/dan/dan_sol-weh-bnacs-unc-chi2-elev_hiweh_sol1-2100.csv', 
            zip(mnames, sols, bf_wehs, unc_wehs, bf_bnacss, unc_bnacss, chi2s, elevs), 
            fmt='%s', 
            delimiter=',')