...
axes[2,0].set_title('Thermal Principal Components (Rover Simulation)')
...
axes[2,1].set_title('Epithermal Principal Components (Rover Simulation)')
...
pca = PCA(n_components=3)
pca.fit(data)
axes[3,0].step(datasets.time_bins_dan[:-1], pca.components_[0][:len(datasets.time_bins_dan)-1], where='post', linewidth=2, label='PC 1')
axes[3,0].step(datasets.time_bins_dan[:-1], pca.components_[1][:len(datasets.time_bins_dan)-1], where='post', linewidth=2, label='PC 2')
axes[3,0].step(datasets.time_bins_dan[:-1], pca.components_[2][:len(datasets.time_bins_dan)-1], where='post', linewidth=2, label='PC 3')
axes[3,0].legend(loc='upper right')
axes[3,0].set_xscale('log')
axes[3,0].set_title('Thermal Principal Components (DAN Actual)')
axes[3,0].set_xlabel('Time (us)')
axes[3,0].set_ylabel('Normalized Counts')
axes[3,1].step(datasets.time_bins_dan[:-1], pca.components_[0][len(datasets.time_bins_dan)-1:], where='post', linewidth=2, label='PC 1')
axes[3,1].step(datasets.time_bins_dan[:-1], pca.components_[1][len(datasets.time_bins_dan)-1:], where='post', linewidth=2, label='PC 2')
axes[3,1].step(datasets.time_bins_dan[:-1], pca.components_[2][len(datasets.time_bins_dan)-1:], where='post', linewidth=2, label='PC 3')
axes[3,1].legend(loc='upper right')
axes[3,1].set_xscale('log')
axes[3,1].set_title('Epithermal Principal Components (DAN Actual)')
axes[3,1].set_xlabel('Time (us)')
axes[3,1].set_ylabel('Normalized Counts')
